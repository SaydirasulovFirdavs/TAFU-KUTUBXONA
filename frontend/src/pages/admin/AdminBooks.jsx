...
function AdminBooks() {
    ...
    const fetchBooks = async () => {
        try {
            setLoading(true);
            const response = await booksAPI.getAll({ limit: 100, status: 'all' });
                        ...
                        {books.map(book => (
                            <tr key={book.id}>
                                ...
                                <td className="col-status">
                                    <span className={`status-badge ${book.status || 'active'}`}>
                                        <span className="status-dot"></span>
                                        {book.status === 'inactive' ? t('admin.inactive') : t('admin.active')}
                                    </span>
                                </td>
