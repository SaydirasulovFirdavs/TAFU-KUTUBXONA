...
const runEmergencySync = async () => {
    try {
        ...
        for (const update of syncUpdates) {
            const result = await query('UPDATE books SET cover_image = $1 WHERE title ILIKE $2', [update.img, `%${update.title}%`]);
            updatedCount += result.rowCount;
        }
        console.log(`--- EMERGENCY DB SYNC COMPLETED: ${updatedCount} records affected ---`);
    } catch (err) {
